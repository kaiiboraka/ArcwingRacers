...
- **Track editor Phase 3 — mesh generation.** The only remaining track-editor piece (ADR 0010 `TrackMeshGenerator`): bake `ROAD`/`TUNNEL` ribbon + walls + tunnel roof from the spline, honoring per-point width/tilt/recipe. Rabbit-hole sized — deliberately parked for a later session. Reference: `https://github.com/iiMidnightii/PathMesh3D` (study only).
