...
- **Spedometer ↔ player via EventBus.** ✅ Done — `Systems/Events/event_bus.gd` autoload (ADR 0002) created + registered in `project.godot`; PodController emits `speed_updated`, `boost_light_changed`, charge/heat + transition signals; spedometer subscribes in `_ready()` (speed number/color, boost light, fill bar); spedometer instanced in `Test_Level.tscn` under `SpedometerLayer` CanvasLayer. Plan: `agent-context/plans/plan-spedometer-eventbus-hookup.md`. Note: `EventBus` now exists — the race-slice plan's "EventBus now-or-later" is resolved (build it now).
