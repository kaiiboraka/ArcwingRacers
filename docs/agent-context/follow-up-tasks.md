...
- **Wing open/close animations (state ladder + turn/pitch/tilt drive).** 🔄 In progress — drive the 4-idle + 2-transition library (`Arcwing.res`) with per-wing independent openness (default rest Open): yaw turn = inside wing squeezes / outside wing opens to Full and holds while held; tilt = air-side wing opens to Full / ground-side closes to Closed; airborne nose-up opens both, nose-down closes both; one state per 0.5s, rendered through the transition animations. Added (2026-08): grounded nose-down squeezes both wings toward Squeezed; `BoostState.CHARGING`/`READY` → both Squeezed, `BOOSTING` → both Full (highest-priority override). Plan: `agent-context/plans/plan-wing-open-animations.md`.

- **Coast deceleration on throttle release.** ✅ Done (needs game test) — added `idle_deceleration : float = 0.5` (fraction of remaining forward speed shed per second while coasting) to `PodController.gd`; `_accelerate` now lerps coasting speed toward 0 so the pod bleeds speed instead of gliding forever (braking stays the deliberate decel).

- **Speed stats standardized to mph.** ✅ Done (needs game test) — `max_speed = 450` mph, `boost_speed_bonus = 111.8`, `boost_thrust = 33.6`, `pitch_speed_gain = 33.6` (exports + `Arcwing.tscn` overrides); converted once to m/s in `_ready()` (`_max_speed_mps` et al.), all internal refs use the m/s twins; `EventBus.speed_updated` now emits **mph** (spedometer displays it directly, no conversion). Note: scene boost values were authored in m/s (100/250) and converted exactly (223.7/559.2 mph) so boost physics are unchanged; cruise ceiling is now 201.2 m/s vs 200.

- **Ramp riding (full terrain-following fix).** ✅ Done (needs game test) — new `@export_category("Terrain")` exports `floor_slope_max_deg = 75` (sets `floor_max_angle` in `_ready`, overriding the scene's 50°), `ground_align = 1.0`, `ground_align_speed = 6.0`; pod up-vector slerps to `_ground_normal` while grounded (`_build_pod_basis` via new `_basis_from_forward_up`), hover spring pushes along the surface normal instead of only `velocity.y`, `_flat_forward()` pitches with the slope so thrust/boost follow the nose while grounded, and `up_direction` is set to the ground normal before `move_and_slide`. `_pitch_attitude_target()` grounded case zeroed (terrain pitch now comes from the basis align; `terrain_pitch_align` is superseded).

- **Ramp snap-back softened.** ✅ Done (needs game test) — per request ("rotation should not so harshly snap back to flat after leaving the ramp"), `_ground_normal` now *retains* its last value when the hover rays miss instead of snapping to `Vector3.UP`, and a new export `ground_align_release_speed = 2.0` (`@export_category("Terrain")`) governs the airborne release rate. `_ground_align` lerps to `ground_align` (grounded, rate `ground_align_speed`) or `0` (airborne, rate `ground_align_release_speed`); `_build_pod_basis` and `_flat_forward()` now gate on `_ground_align > 0.001` (not `_grounded`), so the ramp attitude lingers into the flight arc instead of popping flat.

- **Provisionary repair system.** ✅ Done (needs game test) — hold `Player_Repair` (`InputCollector.repair_held`, already bound) to engage repair: enters only when there is something to repair (active overheat, or any part below `repair_done_threshold`); otherwise the press does nothing. While repairing the pod is slowed to `repair_speed_cap_fraction = 0.75` of base max speed (`repair_deceleration = 3.0`), charging/boosting are blocked (cancels `CHARGING`/`READY`/`BOOSTING` on entry), and `_normal_boost`/`_charge_boost`/`_ready_boost` early-return while repairing. `_apply_repair` works an overheat-repair effort pool (`_overheat_repair_remaining`, armed to `repair_overheat_time = 3.0` on overheat) down a bit per second of holding — progress is kept between presses, releasing mid-way doesn't reset it — and clears the overheat when it hits zero; otherwise it ticks `repair_tick_rate = 0.5` health/sec into the most-damaged part (`_most_damaged_part`, one at a time, up to `repair_done_threshold = 0.8`) and auto-stops when nothing is repairable. Parts = `POD_PART_NAMES = ["Left Wing", "Right Wing", "Chassis", "Engines"]`, all starting at full health (`_part_health`), with `apply_part_damage(part_index, fraction)` as the public hook for the future damage system. New `@export_category("Repair")` block (`PodController.gd:297-314`), state `_repairing`/`_overheat_repair_remaining`, functions `_repair_process`/`_can_repair`/`_most_damaged_part`/`_apply_repair` (~:1014-1076). Repair Rate stat (0.10–1.00 in `pod-stats.md`) will eventually drive `repair_tick_rate`. Repair is wired to HUD as its own EventBus channel (`repair_started`/`repair_ended` — deliberately NOT a boost state, since repair can overlap OVERHEAT): `_set_repairing` emits them; the spedometer shows `repair_light_color` (default orange, `spedometer.gd:52`) while repairing and reverts to the boost-state color on release (`_on_repair_started`/`_on_repair_ended`), with `_on_boost_state_changed` skipping the light while `_repairing`.

- **Rear-view camera (Look Behind) as a PhantomCamera node.** The design (`game-design/controls/controls-and-camera.md` → "Look Behind", input `look_behind` in `input-buffer.md`) calls for a rear-view toggle, but no camera work has been done — the current view is the single forward PhantomCamera. When implementing, add a second PhantomCamera3D in the pod scene (behind + above the pod, facing backward) with `look_at_target`/priority or a toggle path so the `look_behind` input switches to it and back; the spline minimap must mirror to match the reversed camera direction.
