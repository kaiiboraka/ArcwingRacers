...
- **Plan-first, never block on approval.** The moment multi-step work appears, formulate the plan, surface any uncertainties inline, and write it straight into `agent-context/plans/plan-<topic>.md`, then add a reference to that plan in `agent-context/follow-up-tasks.md` — all without waiting for approval or input. Open questions, uncertainties, and options get written *into the plan doc itself* (a dedicated "Questions / Options" section, each item tagged so it can be resolved later). Do not stall execution waiting for answers: proceed with the unambiguous parts, and let the written plan be the record we pick questions back up from — the user can read it and we discuss it in a later session.
