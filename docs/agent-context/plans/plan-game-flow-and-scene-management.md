...
| Main menu / options / pause wiring | `UI/MainMenu/`, `UI/PauseMenu/`, `UI/Opening/`, `UI/LoadingScreen/`, `UI/Shared/` (moved out of `UI/Example_Scenes/` by `[CON]`) | GUI scenes under `GameManager.ui`; New Game → ProjectLoader → test level; `ui_cancel` → pause layer. **Done:** main menu host is the project copy `UI/MainMenu/main_menu.tscn` (`ProjectLoader.MAIN_MENU_SCENE`); opening splash is the project copy `UI/Opening/opening.tscn`+`opening.gd`; New Game → `ProjectLoader.DEFAULT_TRACK_SCENE` (`Level_Grassy.tscn`); HUD mounted by uid (`res://UI/HUD/HUD.tscn` = `uid://c1gnx5bseg8ac`, `ProjectLoader.HUD_SCENE_UID`) |
...
| `UI/MainMenu/main_menu.tscn` | Project main menu host GUI (project copy; not `main_menu_with_animations.tscn`) |
| `UI/Opening/opening.tscn` + `opening.gd` | Project opening splash; handoff → `ProjectLoader.opening_finished()` |
