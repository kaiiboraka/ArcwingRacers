...
## Samples a terrain mesh's triangle soup and returns per-blade spawn data as
## [Transform3D, Color] pairs. Each triangle gets ~area × density blades dropped at a
## random barycentric point (BCC sampling), oriented along the interpolated face normal
## with a random yaw. The Color packs per-instance custom data: x = blade width,
## y = blade height, z = sway-pitch angle (rad), w = sway-yaw angle (rad) — read back in
## the grass shader as INSTANCE_CUSTOM.
...
static func generate(
	...
	for index in range(0, indices.size(), 3):
		var j : int = indices[index]
		var k : int = indices[index + 1]
		var l : int = indices[index + 2]
