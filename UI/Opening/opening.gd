extends Control
## Project copy of Maaack's opening scene. Same fade/skip/loading mechanics as the template,
## but the handoff at the end swaps the UI container through ProjectLoader instead of using
## SceneLoader.change_scene_* (this project swaps scenes inside the master scene's
## containers, never get_tree().change_scene_*).
...
func _on_scene_loaded() -> void:
	ProjectLoader.opening_finished()
...
func _load_next_scene() -> void:
	...
	if status == ResourceLoader.THREAD_LOAD_LOADED:
		...
		ProjectLoader.opening_show_loading_screen()
